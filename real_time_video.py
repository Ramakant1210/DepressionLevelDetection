...
def RealTime():
    ...
    negative = ['angry', 'sad', 'disgust', 'scared']
    negativity = 0
    values = freq.values()
    for i in freq.keys():
        if i in negative:
    ...
    depression_level = negativity / sum(values)
    print(depression_level)
    if(depression_level<0.3):
        level="No Depression"

    elif(0.3<=depression_level<0.6):
        level="Medium Depression"
    ...
    return render_template('RealTime.html',output=depression_level,output1=level)
